...
void MPU_ConfigureRegion(const MPU_RegionInit_TypeDef *init)
{
  ...
  if (init->regionEnable)
  {
    ...
    MPU->RASR = ((init->disableExec ? 1 : 0) << (MPU_RASR_ATTRS_Pos+12)) |
                (init->accessPermission      << (MPU_RASR_ATTRS_Pos+8))  |
                (init->tex                   << (MPU_RASR_ATTRS_Pos+3))  |
                ((init->shareable   ? 1 : 0) << (MPU_RASR_ATTRS_Pos+2))  |
                ((init->cacheable   ? 1 : 0) << (MPU_RASR_ATTRS_Pos+1))  |
                ((init->bufferable  ? 1 : 0) <<  MPU_RASR_ATTRS_Pos)     |
